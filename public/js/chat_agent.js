...
function ResizeConsole () {
    // Layout is handled by CSS flexbox — no JS resize needed
}
...
function _PostMessage (cmd, msg) {
    const isUser = (cmd === "SHOW_SELF_MESSAGE");
    let theMsg = msg.message;
    const md_class = use_markdown ? "kilroy-markdown" : "kilroy-message";
    const md_styles = use_markdown ? "" : "padding-bottom:8px;";
    ...
    let html;
    if (page_mode === "page") {
        html = `<div class="${md_class}" style="${md_styles}">${theMsg}</div>`;
        ClearLog("");
    } else {
        const bubbleClass = isUser ? "chat-bubble chat-bubble-user" : "chat-bubble chat-bubble-bot";
        html = `<div class="${bubbleClass}"><div class="bubble-header"><span class="bubble-name">${msg.username}</span><span class="bubble-time">${moment().format("LTS")}</span></div><div class="bubble-body ${md_class}" style="${md_styles}">${theMsg}</div></div>`;
    }
    logmsg(html);
}
...
const {div, input, textarea} = van.tags;
    ...
    return div({class: "chat-container"},
        ...
        div({class: "chat-input-bar"},
            div({class: "input-name"},
                div({class: "input-label"}, "from"),
                input({id: "username", placeholder: "your name", type: "text", autocomplete: "off"})
            ),
            div({class: "input-msg"},
                textarea({id: "msg", placeholder: "message — Enter to send, Shift+Enter for newline", rows: "2"})
            )
        )
    );
}
